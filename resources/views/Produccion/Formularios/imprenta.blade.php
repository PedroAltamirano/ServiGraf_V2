...
$materiales = App\Models\Produccion\Material::where('empresa_id', Auth::user()->empresa_id)
...
<section id="descripcion">
  <div class="form-row">
    <div class="form-group col-12 col-md-6">
      <div class="form-group">
        ...
        <textarea class="form-control form-control-sm" name="detalle" id="detalle"
          rows="1">{{ old('detalle', $pedido->detalle) }}</textarea>
      </div>
    </div>
    ...
    <div class="form-group col-4 col-md-2">
      ...
      <input type="number" name="cantidad" id="cantidad" class="form-control form-control-sm" min="0"
        value="{{ old('cantidad', $pedido->cantidad) ?? '0' }}">
    </div>
  </div>
  <div class="form-row">
      ...
      <div class="form-row" id="corte">
        <div class="col-6">
          <input type="number" name="corte_ancho" id="corte_ancho" class="form-control form-control-sm fixFloat"
            value="{{ old('corte_ancho', $pedido->corte_ancho) ?? '0.00' }}" min="0" step="0.01"
            aria-describedby="help_corte_ancho">
          <small id="help_corte_ancho" class="text-muted">Ancho</small>
        </div>
        <div class="col-6">
          <input type="number" name="corte_alto" id="corte_alto" class="form-control form-control-sm fixFloat"
            value="{{ old('corte_alto', $pedido->corte_alto) ?? '0.00' }}" min="0" step="0.01"
            aria-describedby="help_corte_alto">
    ...
    <div class="form-group col-12 col-md-5 order-3 order-md-2">
      ...
      <div class="form-row" id="tintas">
        <div class="col-6">
          <select name="tinta_tiro[]" id="tinta_tiro" class="form-control form-control-sm d-print-none"
            aria-describedby="help_tintas_tiro" multiple>
            @foreach ($tintas as $tinta)
              <option value="{{ $tinta->id }}" {{ in_array($tinta->id, $oldTintasTiro) ? 'selected' : '' }}>
                {{ $tinta->color }}</option>
            @endforeach
          </select>
        ...
        <div class="col-6">
          <select name="tinta_retiro[]" id="tinta_retiro" class="form-control form-control-sm"
            aria-describedby="help_tintas_retiro" multiple>
            @foreach ($tintas as $tinta)
              <option value="{{ $tinta->id }}" {{ in_array($tinta->id, $oldTintasRetiro) ? 'selected' : '' }}>
                {{ $tinta->color }}</option>
            @endforeach
          </select>
    ...
    <div class="form-group col-6 col-md-4 order-2 order-md-3">
      ...
      <div class="form-row" id="numerado">
        <div class="col-6">
          <input type="number" name="numerado_inicio" id="numerado_inicio" class="form-control form-control-sm"
            value="{{ old('numerado_inicio', $pedido->numerado_inicio) ?? '0' }}" min="0"
            aria-describedby="help_numerado_inicio">
          <small id="help_numerado_inicio" class="text-muted">Inicial</small>
        </div>
        <div class="col-6">
          <input type="number" name="numerado_fin" id="numerado_fin" class="form-control form-control-sm"
            value="{{ old('numerado_fin', $pedido->numerado_fin) ?? '0' }}" min="0"
            aria-describedby="help_numerado_fin">
  ...
  <table id="table-materiales" class="table table-sm table-responsive">
    ...
    <tfoot>
      <tr class="font-weight-bold">
        <td colspan="2"><a class="text-muted d-print-none" href="http://www.pplanos.com/" target="_blank">Corte de
            papel</a></td>
        <td colspan="4" class="text-right"></td>
        <td colspan="2" class="text-right"><a id="printable" data-target="material" class="d-print-none"><i
              class="fas fa-print pr-2"></i></a> Total material $</td>
        <td class="text-center"><input type="number" name="total_material" id="total_material"
            value="{{ old('total_material', $pedido->total_material) ?? '0.00' }}"
  ...
  <table id="table-procesos" class="table table-sm table-responsive-sm">
    ...
    <tfoot>
      <tr class="font-weight-bold">
        ...
        <td class="text-center"><input type="number" id="total_pedido" name="total_pedido"
            value="{{ old('total_pedido', $pedido->total_pedido) ?? '0.00' }}"
            class="form-control form-control-sm text-center" readonly></td>
        <td></td>
      </tr>
      <tr>
        <td colspan="2" class="text-right">Abonos $</td>
        <td class="text-center"><input type="number" id="abono" name="abono"
            value="{{ old('abono', $pedido->abono) ?? '0.00' }}" class="form-control form-control-sm text-center"
            readonly></td>
        <td><i data-toggle="modal" data-target="#modalAbonos" class="fas fa-eye"></i></td>
      </tr>
      <tr>
        <td colspan="2" class="text-right">Saldo $</td>
        <td class="text-center"><input type="number" id="saldo" name="saldo"
            value="{{ old('saldo', $pedido->saldo) ?? '0.00' }}" class="form-control form-control-sm text-center"
            readonly></td>
        <td></td>
      </tr>
    </tfoot>
...
@section('modals2')
  ...
  <!-- Modal Abonos -->
  <div class="modal fade" id="modalAbonos" tabindex="-1" role="dialog" aria-labelledby="modelTitleId"
    aria-hidden="true">
            ...
            <table id="table-abonos" class="table table-sm table-responsive">
              ...
              <tfoot>
                <tr class="font-weight-bold">
                  ...
                  <td class="text-center"><input id="abonos" class="form-control form-control-sm text-center"
                      type="number" readonly name="abono" value="{{ old('abono', $pedido->abono) ?? '0.00' }}"></td>
                </tr>
              </tfoot>
            </table>
        </div>
...
@section('after.after.scripts')
  <script>
    ...
    const add_material = (material_id = null, material_cantidad = null, material_corte_alt = 0.00, material_corte_anc =
      0.00, material_tamanios = null, material_proveedor = null, material_factura = null, material_total = 0.00) => {
      ...
      let button = $('<i />', {
        ...
        'name': 'remove',
        'id': `material-${index_material}`
      });

      let material = $('<select />', {
        'class': 'form-control form-control-sm select2Class',
        'name': 'material_id[]',
        'id': `material_id_${index_material}`
      }).append(opts_mats);
      ...
      let cantidad = $('<input />', {
        ...
        'id': `material_cantidad_${index_material}`,
        'min': '0',
        'onchange': `sumarMaterial();`
      });

      let corte_alt = $('<input />', {
        'type': 'number',
        'class': 'form-control form-control-sm text-center fixFloat',
        'value': material_corte_alt,
        'name': 'material_corte_alt[]',
        'id': `material_corte_alt_${index_material}`,
        'min': '0',
        'step': '0.01'
      });
      ...
      let corte_anc = $('<input />', {
        'type': 'number',
        'class': 'form-control form-control-sm text-center fixFloat',
        'value': material_corte_anc,
        'name': 'material_corte_anc[]',
        'id': `material_corte_anc_${index_material}`,
        'min': '0',
        'step': '0.01'
      });
      ...
      let tamanios = $('<input />', {
        'type': 'number',
        'class': 'form-control form-control-sm text-center',
        'value': material_tamanios,
        'name': 'material_tamanios[]',
        'id': `material_tamanios_${index_material}`,
        'min': '0'
      });

      let proveedor = $('<select />', {
        'class': 'form-control form-control-sm select2Class',
        'name': 'material_proveedor[]',
        'id': `material_proveedor_${index_material}`
      }).append(opts_provs);
      ...
      let factura = $('<input />', {
        'type': 'number',
        'class': 'form-control form-control-sm text-center',
        'value': material_factura,
        'name': 'material_factura[]',
        'id': `material_factura_${index_material}`,
        'min': '0'
      });
      ...
      newRow(table, [button, material, cantidad, corte_alt, corte_anc, tamanios, proveedor, factura, total],
        `row-material-${index_material}`);
    ...
    if (old_material != []) {
      old_material.map(item => {
        add_material(item.material_id, item.cantidad, item.corte_alto, item.corte_ancho, item.tamanos, item
          .proveedor_id, item.factura, item.total);
      });
    }
    ...
    const add_proceso = (proceso_id = null, proceso_tiro = 1, proceso_retiro = 0, proceso_millar = 1, proceso_valor =
      0.00, proceso_total = 0.00, proceso_terminado = 0) => {
      ...
      let tiro = $('<input />', {
        ...
        'value': proceso_tiro,
        'name': 'proceso_tiro[]',
        'id': `tiro-${index_procesos}`,
        'min': '0',
        'onchange': `sumar(${index_procesos});`
      });

      let retiro = $('<input />', {
        'type': 'number',
        'class': 'form-control form-control-sm text-center',
        'value': proceso_retiro,
        'name': 'proceso_retiro[]',
        'id': `retiro-${index_procesos}`,
        'min': '0',
        'onchange': `sumar(${index_procesos});`
      });
      ...
      let millar = $('<input />', {
        'type': 'number',
        'class': 'form-control form-control-sm text-center',
        'value': proceso_millar,
        'name': 'proceso_millar[]',
        'id': `mill-${index_procesos}`,
        'min': '0',
        'onchange': `sumar(${index_procesos});`
      });
      ...
      let total = $('<input />', {
        ...
        'step': '0.0001',
        'name': 'proceso_total[]',
        'id': `total_proceso-${index_procesos}`,
        'min': '0',
        'onchange': `sumar(${index_procesos});`,
        'readonly': 'readonly'
      });
      ...
      let terminado = $('<input />', {
        ...
        'class': 'form-control form-control-sm text-center',
        'value': 1,
        'name': `proceso_terminado[${index_procesos}]`,
        'id': `proceso_terminado-${index_procesos}`
      }).prop('checked', proceso_terminado == 1 ? true : false);
    ...
    if (old_procesos != []) {
      old_procesos.map(item => {
        add_proceso(item.proceso_id, item.tiro, item.retiro, item.millares, item.valor_unitario, item.total, item
          .status);
      });
    }
    ...
    const add_abono = (abono_fecha = null, abono_usuario = null, abono_usuario_nombre = null, abono_pago = null,
      abono_valor = 0.00) => {
      ...
      let usuario = $('<input />', {
        ...
        'value': abono_usuario,
        'name': 'abono_usuario[]',
        'id': `abono_usuario_${index_abono}`
      });

      let usuario_nombre = $('<input />', {
        'type': 'text',
        'class': 'form-control form-control-sm',
        'value': abono_usuario_nombre,
        'name': 'abono_usuario_nombre[]',
        'id': `abono_usuario_nombre_${index_abono}`,
        'readonly': 'readonly'
      });
      ...
      let valor = $('<input />', {
        ...
        'value': abono_valor,
        'name': 'abono_valor[]',
        'id': `abono_valor_${index_abono}`,
        'min': '0',
        'step': '0.0001',
        'onchange': 'sumarAbonos();'
      });
