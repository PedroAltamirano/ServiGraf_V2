    ...
    <div class="row">
      @foreach ($categorias as $item)
        <div class="col-6 col-md-2">
          <x-crud routeEdit="#modalCat" :modalEdit="$item" :routeDelete="route('categoria.delete', $item->id)"
            :textDelete="$item->categoria" />
          &nbsp;&nbsp;{{ $item->categoria }}
        </div>
      @endforeach
    </div>
    ...
    <table id="tableMat" class="table table-striped table-sm">
      <thead>
        <tr>
          ...
          <th scope="col" class="w-2">Color</th>
          <th scope="col" class="w-2">UV</th>
          <th scope="col" class="w-2">Plastificado</th>
      ...
      <tbody>
        @foreach ($materiales as $item)
          <tr>
            ...
            <td>
              <x-crud :routeEdit="route('material.edit', $item->id)" :routeDelete="route('material.delete', $item->id)"
                :textDelete="$item->descripcion" />
            </td>
          </tr>
        @endforeach
      </tbody>
  ...
  <x-blue-board title='Tintas' :foot="[ ['text'=>'Nuevo', 'href'=>'#modalTinta', 'id'=>'nuevo', 'tipo'=>'modal'] ]">
    <div class="row">
      @foreach ($tintas as $item)
        <div class="col-6 col-md-2">
          <x-crud routeEdit="#modalTinta" :modalEdit="$item" :routeDelete="route('tinta.delete', $item->id)"
            :textDelete="$item->color" />
          &nbsp;&nbsp;{{ $item->color }}
        </div>
      @endforeach
    </div>
