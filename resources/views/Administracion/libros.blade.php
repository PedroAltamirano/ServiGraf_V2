    ...
    <div class="col-12 col-md form-group">
      ...
      <select name="usuario" id="usuario" class="form-control form-control-sm">
        ...
        @foreach ($usuarios as $user)
          <option value="{{ $user->cedula }}" {{ Auth::id() == $user->cedula ? 'selected' : '' }}>
            {{ $user->usuario }}</option>
        @endforeach
      </select>
    </div>
  ...
  <script>
    ...
    var table = $('#table').DataTable({
      ...
      "columns": [{
        ...
        },
        {
          ...
          "render": (data, type, full, meta) => {
            ...
            crud +=
              `<a class='fa fa-print' href='#modalRecibo' data-toggle='modal' data-entrada='${dataJson}'></a>`;
            return crud;
          }
        }
      ],
      ...
      "footerCallback": function(row, data, start, end, display) {
        ...
        let ingTotal = api.column('ingreso:name', {
          search: 'applied'
        }).data().reduce((a, b) => intVal(a) + intVal(b), 0);
        let egrTotal = api.column('egreso:name', {
          search: 'applied'
        }).data().reduce((a, b) => intVal(a) + intVal(b), 0);
        ...
        if (ingTotal < egrTotal) $("#total-saldo").css({
          'color': "red"
        });
