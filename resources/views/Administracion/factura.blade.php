...
@section('desktop-content')
  <x-path
    :items="[ ['text' => 'Facturas', 'current' => false, 'href' => route('facturacion')], ['text' => $text, 'current' => true, 'href' => '#'] ]">
    <li class="breadcrumb-item" id="fact_num_path">
      <input type="number" name="number_vis" id="fact_num_vis" class="form-control form-control-sm"
        value="{{ $fact_num }}" readonly>
    </li>
  </x-path>
  ...
  <x-blue-board :title=$text :foot="[ ['text'=>$action, 'href'=>'#', 'id'=>'formSubmit', 'tipo'=>'link'] ]">
    <form action="{{ $path }}" method="POST" id="form">
        ...
        <div class="form-row">
          <div class="form-group col-12 col-md-3">
            ...
            <select class="form-control form-control-sm select2Class @error('cliente_id') is-invalid @enderror"
              name="cliente_id" id="cliente" data-tags="true">
          ...
          <div class="form-group col-6 col-md-2">
            ...
            <input type="text" id="ruc" name="ruc" class="form-control form-control-sm" value="{{ old('ruc') }}"
              readonly>
          </div>
          <div class="form-group col-6 col-md-2">
          ...
          <div class="form-group col-6 col-md-2">
            ...
            <input type="date" name="emision" id="emision"
              class="form-control form-control-sm @error('emision') is-invalid @enderror"
              value="{{ old('emision', $factura->emision) ?? date('Y-m-d') }}">
          </div>
          <div class="form-group col-6 col-md-2">
            <label for="inicio">Vencimiento</label>
            <input type="date" name="vencimiento" id="vencimiento"
              class="form-control form-control-sm @error('vencimiento') is-invalid @enderror"
              value="{{ old('vencimiento', $factura->vencimiento) ?? date('Y-m-d') }}">
          </div>
            ...
            <select name="fact_emp_id" id="fact_emp_id" class="form-control form-control-sm refresh">
              @foreach ($empresas as $item)
                <option value="{{ $item->id }}"
                  {{ old('fact_emp_id', $factura->fact_emp_id) == $item->id ? 'selected' : '' }}>
                  {{ $item->empresa }}</option>
              @endforeach
            </select>
          </div>
        ...
        <table id="table-articulos" class="table table-sm">
          <thead>
            <tr>
              ...
              <th scope="col" class="w-50">Detalle</th>
              <th scope="col" class="w-10">% IVA</th>
              <th scope="col" class="w-10 text-right">Precio Unitario</th>
              <th scope="col" class="w-10 text-right">Subtotal</th>
            </tr>
          </thead>
          <tbody>
          </tbody>
          <tfoot>
            ...
            <tr class="font-weight-bold">
              ...
              <td><input type="number" name="iva" id="iva" value="{{ old('iva', $factura->iva) ?? '0.00' }}"
                  class="form-control form-control-sm text-right fixFloat" readonly></td>
            </tr>
            <tr class="font-weight-bold">
              <td colspan="3"></td>
              <td colspan="2" class="text-right">IVA 0 $</td>
              <td><input type="number" name="iva_0" id="iva_0" value="{{ old('iva_0', $factura->iva_0) ?? '0.00' }}"
                  class="form-control form-control-sm text-right fixFloat" readonly></td>
            </tr>
            <tr class="font-weight-bold">
              <td colspan="3"></td>
              <td colspan="2" class="text-right">Total $</td>
              <td><input type="number" name="total" id="total" value="{{ old('total', $factura->total) ?? '0.00' }}"
                  class="form-control form-control-sm text-right fixFloat" readonly></td>
            </tr>
          </tfoot>
        </table>
      ...
      <section id="retenciones">
        <table id="table-retenciones" class="table table-sm table-borderless">
          <tbody>
            <tr>
              <td scope='col' class="w-10">Retenciones</td>
                  ...
                  <div class="form-group col form-inline d-flex justify-content-end">
                    ...
                    <select class="form-control form-control-sm mx-2 retencion" name="ret_iva_p" id="ret_iva_p">
                      @foreach ($ret_iva as $ret)
                        <option value="{{ $ret->id }}"
                          {{ old('ret_iva_p', $factura->ret_iva_p) == $ret->id ? 'selected' : '' }}>
                          {{ $ret->porcentaje }}
                        </option>
                      @endforeach
                    </select>
                  ...
                  <div class="form-group col form-inline d-flex justify-content-end">
                    ...
                    <select class="form-control form-control-sm mx-2 retencion" name="ret_fuente_p" id="ret_fnt_p">
                      @foreach ($ret_fnt as $ret)
                        <option value="{{ $ret->id }}"
                          {{ old('ret_fuente_p', $factura->ret_fuente_p) == $ret->id ? 'selected' : '' }}>
              ...
              <td scope='col' class="w-10 text-right">Total a cobrar $</td>
              <td scope='col' class="w-10">
                <input type="number" class="form-control form-control-sm text-right fixFloat"
                  value="{{ old('total_pagar', $factura->total_pagar) ?? '0.00' }}" name="total_pagar" id="tot_cob"
                  readonly>
              </td>
            </tr>
          </tbody>
      ...
      {{-- <section id="retenciones">
        <div class="row m-0">
          ...
          <div class="col-12 col-md form-inline p-0">
            <div class="form-group w-100 m-0">
              ...
              <select class="form-control form-control-sm mx-2 retencion" name="ret_iva_p" id="ret_iva_p">
                @foreach ($ret_iva as $ret)
                  <option value="{{ $ret->id }}"
                    {{ old('ret_iva_p', $factura->ret_iva_p) == $ret->id ? 'selected' : '' }}>{{ $ret->porcentaje }}
                  </option>
                @endforeach
              </select>
              <label for="ret_iva">%:</label>
              <input type="number" class="form-control form-control-sm mx-2 w-25 text-right fixFloat"
                value="{{ old('ret_iva', $factura->ret_iva) ?? '0.00' }}" name="ret_iva" id="ret_iva" readonly>
            </div>
          </div>
          <div class="col-12 col-md form-inline p-0">
          ...
          <div class="col-12 col-md form-inline p-0 border">
            <div class="form-group w-100 m-0">
              <label for="tot_cob">Total a cobrar $</label>
              <input type="number" class="form-control form-control-sm mx-2 w-50 text-right fixFloat"
                value="{{ old('total_pagar', $factura->total_pagar) ?? '0.00' }}" name="total_pagar" id="tot_cob"
                readonly>
            </div>
          </div>
        ...
        <div class="form-group">
          ...
          <select multiple class="form-control select2Class" name="pedidos[]" id="pedidos">
            @foreach ($pedidos as $pedido)
              <option value="{{ $pedido->id }}" {{ in_array($pedido->id, $pedidos_old) ? 'selected' : '' }}>
                {{ $pedido->numero }}</option>
            @endforeach
          </select>
        </div>
      </section>
      ...
      <div class="form-group">
        ...
        <textarea class="form-control form-control-sm" name="notas" id="notas"
          rows="2"> {{ old('notas', $factura->notas) }} </textarea>
      </div>
      </section>
    </form>
  </x-blue-board>
...
@section('scripts')
  <script>
    ...
    const add_producto = (cantidad_val = 0, detalle_val = null, iva_id_val = null, valor_unitario_val = '0.00',
      subtotal_val = '0.00') => {
      ...
      let detalle = $('<input />', {
        ...
        'id': `articulo_detalle_${i}`,
        'value': detalle_val
      });

      let iva = $('<select />', {
        'name': 'articulo_iva_id[]',
        'id': `articulo_iva_${i}`,
        'class': 'form-control form-control-sm text-center',
        'onchange': `sumar(${i});`
      }).append(opts_ivas);
      ...
      let unitario = $('<input />', {
        ...
        'value': valor_unitario_val,
        'name': 'articulo_valor_unitario[]',
        'id': `articulo_valor_unitario_${i}`,
        'min': '0',
        'onchange': `sumar(${i});`
      });
      ...
      let subtotal = $('<input />', {
        ...
        'class': 'form-control form-control-sm text-right fixFloat',
        'value': subtotal_val,
        'name': 'articulo_subtotal[]',
        'id': `articulo_subtotal_${i}`,
        'readonly': 'readonly',
        'min': '0'
      });
