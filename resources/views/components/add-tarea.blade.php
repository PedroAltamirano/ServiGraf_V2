      ...
      <div class="modal-body">
        <form action="{{ route('crm.store') }}" id="tareaForm" method="post">
          ...
          <section id="datos-contacto">
            <h6><i class="fas fa-plus" data-toggle="modal" data-target="#modalContacto"></i>&nbsp; Datos del contacto
            </h6>
            <hr>
            <div class="form-row">
              <div class="form-group col-12 col-md-6">
                <label for="contacto_id">Contacto</label>
                <select class="form-control form-control-sm @error('contacto_id') is-invalid @enderror select2Class"
                  name="contacto_id" id="contacto_id" data-tags="true">
                  <option disabled selected>Selecciona uno...</option>
                  @foreach ($empresas as $item)
                    <optgroup label="{{ $item->nombre }}">
                      @foreach ($item->contactos as $item)
                        <option value="{{ $item->id }}" {{ old('actividad_id') == $item->id ? 'selected' : '' }}>
              ...
              <div class="form-group col-12 col-md-6">
                ...
                <input type="text" id="task_organizacion" class="form-control form-control-sm"
                  value="{{ old('organizacion') }}" readonly>
              </div>
              <div class="form-group col-12 col-md-4">
                <label for="telefono">Telefono</label>
                <input type="text" id="task_telefono" class="form-control form-control-sm"
                  value="{{ old('telefono') }}" readonly>
              </div>
              <div class="form-group col-12 col-md-4">
                <label for="email">Email</label>
                <input type="text" id="task_email" class="form-control form-control-sm" value="{{ old('email') }}"
                  readonly>
              </div>
              <div class="form-group col-12 col-md-4">
                <label for="direccion">Dirección</label>
                <input type="text" id="task_direccion" class="form-control form-control-sm"
          ...
          <section id="tarea">
            <div class="form-row">
              <div class="form-group col-12 col-md-6">
                ...
                <select class="form-control form-control-sm  @error('actividad_id') is-invalid @enderror"
              ...
              <div class="form-group col-12 col-md-6">
                ...
                <select class="form-control form-control-sm  @error('asignado_id') is-invalid @enderror"
                  name="asignado_id" id="asignado_id">
                  <option disabled selected>Selecciona uno</option>
                  @foreach ($usuarios as $item)
                    <option value="{{ $item->cedula }}" {{ old('asignado_id') == $item->id ? 'selected' : '' }}>
                      {{ $item->usuario }}</option>
                  @endforeach
                </select>
              </div>
              ...
              <div class="form-group col-12 col-md-4">
                ...
                <input type="time" class="form-control form-control-sm  @error('hora') is-invalid @enderror" name="hora"
                  id="hora" value="{{ old('hora') }}">
              </div>
              <div class="form-group col-12">
                <label for="nota">Nota</label>
                <textarea class="form-control form-control-sm  @error('nota') is-invalid @enderror" name="nota"
                  id="nota" rows="3">{{ old('nota') }}</textarea>
              </div>
            </div>
          </section>
        </form>
