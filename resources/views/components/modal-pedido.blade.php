<!-- Modal Pedido -->
<div class="modal fade" id="modalPedido" tabindex="-1" role="dialog" aria-labelledby="modelTitleId"
  aria-hidden="true">
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <a class="fas fa-print" id="printer" data-target="pedido-container"></a>
      </div>
    </div>
  </div>
</div>
...
@push('component-script')
  <script>
    const routeAjaxPedido = `{{ route('pedido.modal', 0) }}`;
    ...
    const getModal = pedido_id => {
      axios
        .get(routeAjaxPedido.replace('/0', `/${pedido_id}`))
        .then(res => {
          let data = res.data;
          populate_modal(data);
        }).catch(error => {
