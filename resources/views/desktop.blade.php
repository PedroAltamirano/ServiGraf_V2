...
<div class="m-2 m-md-3 row">
  ...
  $sa = $servicios->where('tipo', 1)->pluck('id')->toArray();
  $items = App\Models\Produccion\Pedido_servicio::select('servicio_id', DB::raw('sum(total) as totalData'))->whereIn('pedido_id', $pa)->whereIn('servicio_id', $sa)->groupBy('servicio_id')->get()->each(function($i){return $i->nombre = $i->servicio->servicio; });
...
<div class="m-2 m-md-3 row">
  ...
  @php
  $pa = $pedidos->map(function($p){return $p->id;})->toArray();
  $sa = $servicios->where('tipo', 0)->pluck('id')->toArray();
  $items = App\Models\Produccion\Pedido_servicio::select('servicio_id', DB::raw('sum(total) as totalData'))->whereIn('pedido_id', $pa)->whereIn('servicio_id', $sa)->groupBy('servicio_id')->get()->each(function($i){return $i->nombre = $i->servicio->servicio; });
