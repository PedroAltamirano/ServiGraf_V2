  ...
  <form action='{{ $path }}' method="POST" id="form">
    ...
    <div class="form-row">
      <div class="form-group col-10 col-md-5 order-1">
        ...
        <input type="text" name="perfil" id="perfil" class="form-control @error('perfil') is-invalid @enderror" placeholder="Nombre del perfil" aria-describedby="helpId" value="{{ old('perfil') ? old('perfil') : session('current.perfil') }}">
      </div>
      <div class="form-group col-12 col-md-6 order-3 order-md-2">
        <label for="descripcion">Descripcion</label>
        <input type="text" name="descripcion" id="descripcion" class="form-control @error('descripcion') is-invalid @enderror" placeholder="Descripcion del perfil" aria-describedby="helpId" value="{{ old('descripcion') ? old('descripcion') : session('current.descripcion') }}">
      </div>
        ...
        <tr>
          ...
          <td>
            <div class="custom-control custom-switch d-flex justify-content-center">
              ...
              {{ (old($item->id.'-1') ? old($item->id.'-1') : session('current.'.$item->id.'-1')) ? 'checked':'' }}>
          ...
          <td>
            <div class="custom-control custom-switch d-flex justify-content-center">
              ...
              {{ (old($item->id.'-2') ? old($item->id.'-2') : session('current.'.$item->id.'-2')) ? 'checked':'' }}>
          ...
          <td>
            <div class="custom-control custom-switch d-flex justify-content-center">
              ...
              {{ (old($item->id.'-3') ? old($item->id.'-3') : session('current.'.$item->id.'-3')) ? 'checked':'' }}>
          ...
          <td>
            <div class="custom-control custom-switch d-flex justify-content-center">
              ...
              {{ (old($item->id.'-4') ? old($item->id.'-4') : session('current.'.$item->id.'-4')) ? 'checked':'' }}>
